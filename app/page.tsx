...
export default function Home() {
  ...
  const [experienceMeta, setExperienceMeta] = useState<{
    mode?: "local" | "hosted";
  ...
  useEffect(() => {
    async function loadExamples() {
      try {
        ...
        // Ignore examples load failure in the main UX.
      }
    }
  ...
  const handleSearch = async (input: string) => {
    ...
    try {
      ...
      setExperienceMeta({
        ...
        dailyLimit: data.experienceDailyLimit,
      ...
      <section id="experience" className="mx-auto max-w-6xl px-4 pb-8">
        <div className="mx-auto mb-4 max-w-2xl rounded-lg border border-[#2d3343]/50 bg-[#11141c] px-4 py-3 text-xs text-[#8b92a5]">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span>体验版目标：快速验证需求，不替代本地开源版。</span>
            {experienceMeta.mode === "hosted" ? (
              <span className="rounded-full bg-primary-500/10 px-2.5 py-1 text-primary-300">
                今日剩余 {experienceMeta.remaining ?? 0} / {experienceMeta.dailyLimit ?? 0}
              </span>
            ) : (
        ...
        <section className="mx-auto max-w-6xl px-4 pb-10">
          <div className="mb-4 flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 text-xl font-semibold text-[#e4e7ee]">内置案例库</h2>
            </div>
            <p className="max-w-md text-right text-xs leading-6 text-[#5c6378]">
              体验版适合先拿一个相近案例试水，再把自己的真实需求补进来。
            </p>
          </div>
      ...
      {isLoading && (
        <section className="mx-auto max-w-6xl px-4 pb-8 text-center">
          <div className="glass-card inline-flex flex-col items-center gap-3 p-8">
            ...
            <p className="text-sm text-[#8b92a5]">正在统一执行侦查流程：需求分析、搜索、评分、报告生成...</p>
          </div>
        </section>
      )}
        ...
        <section className="mx-auto max-w-6xl space-y-4 px-4 pb-16">
          ...
          {devPromptResult?.summary && (
            <div className="rounded-lg border border-l-2 border-[#2d3343]/50 border-l-primary-500/60 bg-[#1a1d27]/50 px-5 py-4">
              <h3 className="mb-3 text-sm font-medium text-[#8b92a5]">侦探结论</h3>
          ...
          {repos.length > 0 && (
            <div>
              <div className="mb-3 flex items-center justify-between px-1">
                ...
                <span className="text-xs text-[#5c6378]">体验版默认展示 Top 10</span>
              </div>
